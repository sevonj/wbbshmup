...
void CameraRigOrbit::_ready() {
	...
	set_name("camera_orbit");
	setup_rig();
}

void CameraRigOrbit::_process(double delta) {
	_process_controller_input(delta);
	_process_camera_arm(delta);
}

void CameraRigOrbit::_process_controller_input(double delta) {
	...
	float z = input->get_axis("zoom_in", "zoom_out");
	...
	pivot->rotate(camera->get_global_basis().get_column(1), x * delta * input_controller_sens.x);
	pivot->rotate(camera->get_global_basis().get_column(0), y * delta * input_controller_sens.y);
	distance += delta * z * distance * 4.;
	distance = Math::clamp(distance, MIN_DIST, MAX_DIST);
}

void CameraRigOrbit::_process_camera_arm(double delta) {
	/*
...
void CameraRigOrbit::_input(const Ref<InputEvent> &event) {
	/*if (!enabled) {
		return;
	}
	// TODO: paused guard
