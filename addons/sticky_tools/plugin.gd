...
var enemy_gizmo = preload("res://addons/sticky_tools/classes/enemy_gizmo.gd").new()
...
var tool_pdm_inspector = preload("res://addons/sticky_tools/classes/tool_path_deform_mesh_inspector.gd").new()
var tool_spm_gizmo = preload("res://addons/sticky_tools/classes/tool_stage_path_marker_gizmo.gd").new()


func _enter_tree():
	add_node_3d_gizmo_plugin(enemy_gizmo)
	add_node_3d_gizmo_plugin(stage_gizmo)
	add_node_3d_gizmo_plugin(tool_spm_gizmo)
	add_inspector_plugin(stage_inspector)
	add_inspector_plugin(tool_pdm_inspector)


func _exit_tree():
	remove_node_3d_gizmo_plugin(enemy_gizmo)
	remove_node_3d_gizmo_plugin(stage_gizmo)
	remove_node_3d_gizmo_plugin(tool_spm_gizmo)
	remove_inspector_plugin(stage_inspector)
	remove_inspector_plugin(tool_pdm_inspector)
